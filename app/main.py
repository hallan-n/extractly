import pdfplumber 
from models import Word, Page, Template, Box
...
def extract_text_to_box(words: list[Word], box: Box) -> str:
    result = []
    for word in words:
        if (
            box.x <= word.x0 <= box.x + box.width and
            box.y <= word.top <= box.y + box.height
        ):
...
def process_pdf(template: Template, pdf_path: str):
    ...
    for box in template.fields:
        model.update({box.name: extract_text_to_box(pages[box.page].words, box)})
...
template = Template(
    name="Rescisão",
    fields=[
        Box(name="cnpj", page=0, x=43, y=71, width=91, height=13),
...
print(process_pdf(template, "/home/neves/Documentos/extractly/RESCISÃO DE CONTRATO.pdf"))
